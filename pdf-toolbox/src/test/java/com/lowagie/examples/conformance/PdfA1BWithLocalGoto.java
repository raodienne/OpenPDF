...
/**
 * Creates a document breaking PDF/A-1b conformance with LocalGoto
 * 
 * @author raodienne
 */

public class PdfA1BWithLocalGoto {
    ...
    public static void main(String[] args) {
        System.out.println("PDF/A-1b with LocalGoto");
        ...
        try {
            ...
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("PdfA1b-with-LocalGoto.pdf"));
            ...
            BaseFont notoBase = BaseFont.createFont(
                    Objects.requireNonNull(PdfA1BWithLocalGoto.class.getClassLoader().getResource(fontDir+"noto/NotoSans-Regular.ttf")).getFile(),
                    BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
            Font noto = new Font(notoBase, 10, Font.NORMAL);
            Font notoRed = new Font(notoBase, 10, Font.NORMAL, new Color(255, 0, 0));

            // a paragraph
            Paragraph p1 = new Paragraph("This document is NOT compliant with PDF-A/1b requirements because of the LocalGoto on ", noto);

            // a chunk with local goto
            Chunk thisWord = new Chunk("this word", notoRed);

            // adding the attributes 'F' does not fix the issue
            HashMap<String, Object> attributes = new HashMap<>();
            attributes.put("F", new PdfNumber(FLAGS_PRINT));
            thisWord.setChunkAttributes(attributes);

            p1.add(thisWord.setLocalGoto("test"));
            ...
            Paragraph p3 = new Paragraph("This paragraph contains a ", noto);
            p3.add(new Chunk("local destination", notoRed).setLocalDestination("test"));
